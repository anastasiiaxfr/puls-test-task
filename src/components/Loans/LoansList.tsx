import { Tabs, Tab, Box, Card, Grid, Hidden, Typography, useMediaQuery } from '@mui/material';
...
const LoansList: React.FC = () => {
  ...
  const customTabsItems = loanRequests.filter(
    (loan: any) =>
      loan.status === 'pending settlement' || loan.status === 'to be disbursed',
  );

  const filteredLoans =
    selectedTab === 'all'
      ? loanRequests
      : loanRequests.filter((loan: any) => loan.status === selectedTab);
  ...
  return (
    <>
      ...
      <Box sx={{ width: '100%' }}>
      <Box mb={5}>
        <Tabs className={styles.loansListTabs} value={selectedTab} onChange={handleChange} aria-label="secondary tabs example">
          <Tab
            key={0}
            value="all"
            label={`all (${loanRequests.length})`}
          />
          {tabOrder.map((tabLabel, index) => {
            const customTabs =
              tabLabel === 'Pending';
            ...
            if (customTabs) {
              return (
                <Tab
                  key={index}
                  value={tabLabel}
                  label={`${tabLabel} (${customTabsItems.length})`}
                />
              );
            } else {
              const loanCount = loanRequests.filter((loan: any) => loan.status === tabLabel).length;
              return (
                <Tab
                  key={index}
                  value={tabLabel}
                  label={`${tabLabel} (${loanCount})`}
                />
              );
            }
          })}
      ...
      { selectedTab === 'Pending' ? customTabsItems.map((loan: any) => (
     ...
     <Grid container alignItems={mobile ? 'flex-start' : 'center'}>
       ...
       <Hidden smDown>
        ...
        </Card>)) : filteredLoans.map((loan: any) => (
        <Card className={styles.loanCard} key={loan.id}>
          <Grid container alignItems={mobile ? 'flex-start' : 'center'}>
            <Grid item xs={7} sm={2}>
              <span className={styles.name}>
